...
for step in range(min_replay):
    ...
    if np.mean(reward_buffer) > best_reward:
        ...
        torch.save(online_net.state_dict(),'./DQN_best_model.pth')
    ...
    if step % 1000 == 0:
        torch.save(target_net.state_dict(),'./DQN_trained_model.pth')
        torch.save(online_net.state_dict(),'./DQN_trained_model_1.pth')
        with open('DQN_max_value.npy', 'wb') as f:
